...
class ProContentVideoGenerator(BaseContentVideoGenerator):
  ...
  def generate_subcontent(self, content_video:VideoContent, subcontent_type:SubcontentType):    
    methods = {
        ...
        SubcontentType.PERSPECTIVE.value: lambda: setattr(content_video, 'critical_perspective', self._generate_critical_perspective(content_video.text, perspective="investigacion")),
        SubcontentType.SOURCES.value: lambda: setattr(content_video, 'sources', self._generate_sources(content_video.text))
    }
  ...
  def generate_content_video(self, video_url:str) -> VideoContent:
    ...
    summary = self.get_summary(formatted_subtitles, perspective="una prediccion economica")
    questions = self.get_questions(formatted_subtitles, perspective="investigacion")
    answers = self.get_answers(questions)
    topics = self.get_topics(formatted_subtitles, perspective="investigacion")
    comments = self._get_comments(formatted_subtitles, perspective="economica")
    critical_perspective = self._generate_critical_perspective(formatted_subtitles, perspective="investigacion")
    sources = self._generate_sources(formatted_subtitles)
    return VideoContent(
      ...
      critical_perspective=critical_perspective,
      sources=sources
    )
  ...
  def get_summary(self, subtitles:str, perspective:str) -> str:
    ...
    El resumen debe ser de {self.AMOUNT_SUMMARY} palabras maximo
  ...
  def get_questions(self, subtitles:str, perspective:str) -> list[str]:
    ...
    """
    prompt = f"""
    Genera {self.AMOUNT_QUESTIONS} preguntas del video utilizando los subtitulos que te voy a pasar
    Con un enfoque de {perspective}
    El formato debe seguir el formato:
    {json_format}
    Subtitulos: {subtitles}
  ...
  def get_answers(self, questions:list[str]) -> list[str]:
    ...
    """
    prompt = f"""
    Genera {self.AMOUNT_ANSWERS} respuestas a las preguntas que te voy a pasar
    El formato debe seguir el formato:
    {json_format}
    Preguntas: {questions}
    """
  ...
  def get_topics(self, subtitles:str, perspective:str) -> list[str]:
    ...
    """
    prompt = f"""
    Genera {self.AMOUNT_TOPICS} temas del video utilizando los subtitulos que te voy a pasar
    Con un enfoque de {perspective}
    El formato debe seguir el formato:
    {json_format}
    Subtitulos: {subtitles}
  ...
  """
  Objetivo:
  Generar perspectiva del video
  Solo va a cambiar cuando la logica de generacion de perspectiva cambie
  """
  def _generate_critical_perspective(self, subtitles: list, perspective:str) -> list[str]:
    prompt = f"""
    Generar una perspectiva {perspective} del video utilizando los subtitulos que te voy a pasar
    Como minimo {self.AMOUNT_PERSPECTIVE} palabras
    Subtitulos: {subtitles}
    """
    response = self._send_prompt(prompt, is_json=False)
    return response
  ...
  """
  Objetivo:
  Generar opciones de fuentes de informacion del video
  Solo va a cambiar cuando la logica de generacion de fuentes de informacion cambie
  """
  def _generate_sources(self, subtitles: list) -> list[str]:
    json_format = """
    [{
      "source": "fuente de informacion del video"
    }]
    """
    prompt = f"""
    Generar {self.AMOUNT_SOURCES} fuentes de informacion del video utilizando los subtitulos que te voy a pasar
    El formato debe seguir el formato:
    {json_format}
    Subtitulos: {subtitles}
    """
