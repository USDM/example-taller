...
class FreeContentTweetGenerator(BaseContentGenerator):
  ...
  def generate_content(self, source_path:str) -> Content:
    text_tweet = self._format_text(source_path)
  ...
  """
  def _format_text(self, source_path:str):
    prompt = f"""
    Formatea el texto de un tweet en html que sea legible para el usuario
    """
  ...
  def _get_comments(self, source_path:str, perspective:str) -> list[ContentComment]:
    ...
    """
    prompt = f"""
    Analiza el tweet que te voy a pasar y genera {self.AMOUNT_COMMENTS} comentarios del tweet
  ...
  def _send_prompt(self, prompt:str, source_path:str, is_json:bool) -> str:
    message = self.ia.create_user_message_with_image(prompt, source_path)
    response = self.ia.send_prompt_with_images([message], is_json=is_json)
